...
//API setup
const API_KEY = "AIzaSyBNuKdskRZRbtiKGWOBremowzPjb3lmRFo";
...
generateBotResponses = async (incomingMessageDiv) => {
    const messageElement = incomingMessageDiv.querySelector(".message-text");
    //add user message to hat history
    chatHistory.push({
        role:"user",
    ...
    try{
        ...
        //extract and display bot responses text
        const apiResponseText = data.candidates[0].content.parts[0].text.replace(/\*\*(.*?)\*\*/g, "$1").trim();
        messageElement.innerText=apiResponseText;
        //add bot responses to hat history
        chatHistory.push({
            role:"model",
...
const HandleOutgoingMessage = (e) => {
    ...
    //create and display user messsage
    ...
    chatBody.scrollTo({ top:chatBody.scrollHeight, behavior: "smooth" });
    //simulate not response with thinking indicator after the delay
    setTimeout(() => {
...
messageInput.addEventListener("keydown", (e) => {
    ...
    if(e.key ==="Enter" && userMessage && userMessage && !e.shiftKey && window.innerWidth > 760) {
